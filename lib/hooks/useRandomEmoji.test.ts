...
import { describe, expect, test, afterEach, beforeEach, mock, jest } from 'bun:test';
import { useRandomEmoji, DEFAULT_EMOJIS, DEFAULT_INTERVAL } from './useRandomEmoji';
...
describe('useRandomEmoji', () => {
  ...
  test('sets up interval with correct timing', () => {
    ...
    expect(mockSetInterval.mock.calls[0][1]).toBe(interval);
  });
