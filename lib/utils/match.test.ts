...
describe('Match Utilities', () => {
  ...
  describe('getQualificationText', () => {
    ...
    test('returns "Needs Improvement" for scores below 30', () => {
      expect(getQualificationText(0)).toBe('Needs Improvement');
      expect(getQualificationText(15)).toBe('Needs Improvement');
      expect(getQualificationText(29)).toBe('Needs Improvement');
    });
  });
});
