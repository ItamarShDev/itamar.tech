...
export default function Fireworks() {
  ...
  const explodeFirework = (firework: Firework, fastExplosion = false) => {
  ...
  const explodeAllFireworks = (force = false) => {
  ...
  const launchFirework = () => {
    ...
    fireworksRef.current.push({
      ...
      vy: -Math.min(
        Math.random() * 1.6 + 4.8,
        Math.sqrt(2 * 0.08 * (canvas.height - targetY)),
      ), // 20% slower upward velocity
      color,
      trail: [],
      particles: [],
      exploded: false,
